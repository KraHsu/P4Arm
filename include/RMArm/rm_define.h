...
typedef struct {
  ...
  int port;     ///< 广播的端口号
  int force_coordinate;  ///< 系统外受力数据的坐标系，-1不支持力传感器 0为传感器坐标系 1为当前工作坐标系
                         ///< 2为当前工具坐标系
...
typedef struct {
  int free_axes
      [6];  ///< 自由驱动方向[x,y,z,rx,ry,rz]，0-在参考坐标系对应方向轴上不可拖动，1-在参考坐标系对应方向轴上可拖动
...
typedef struct {
  ...
  int control_mode[6];  ///<  6个力控方向（Fx Fy Fz Mx My Mz）的模式 0-固定模式 1-浮动模式 2-弹簧模式 3-运动模
                        ///<  4-力跟踪模式 8-力跟踪+姿态自适应模式
  float desired_force[6];  ///< 力控轴维持的期望力/力矩，力控轴的力控模式为力跟踪模式时，期望力/力矩设置才会生效
                           ///< ，精度0.1N。
...
typedef struct {
  ...
  rm_pose_t
      pose;  ///< 当前坐标系下的目标位姿，支持四元数/欧拉角表示姿态。位置精度：0.001mm，欧拉角表示姿态，姿态精度：0.001rad，四元数方式表示姿态，姿态精度：0.000001
  float joint[ARM_DOF];  ///< 目标关节角度，单位：°，精度：0.001°
  int sensor;            ///< 传感器，0-一维力；1-六维力
  int mode;              ///< 0-基坐标系力控；1-工具坐标系力控；
  bool follow;           ///< 表示驱动器的运动跟随效果，true 为高跟随，false 为低跟随。
  int control_mode[6];  ///< 6个力控方向的模式 0-固定模式 1-浮动模式 2-弹簧模式 3-运动模式 4-力跟踪模式 5-浮动+运动模式
                        ///< 6-弹簧+运动模式 7-力跟踪+运动模式 8-姿态自适应模式
  float desired_force[6];  ///< 力控轴维持的期望力/力矩，力控轴的力控模式为力跟踪模式时，期望力/力矩设置才会生效
                           ///< ，精度0.1N。
  float limit_vel[6];      ///< 力控轴的最大线速度和最大角速度限制，只对开启力控方向生效。
...
typedef struct {
  ...
  int error;  ///< 夹爪错误信息，低8位表示夹爪内部的错误信息bit5-7 保留bit4 内部通bit3 驱动器bit2 过流 bit1 过温bit0
              ///< 堵转
  int mode;  ///< 当前工作状态：1 夹爪张开到最大且空闲，2 夹爪闭合到最小且空闲，3 夹爪停止且空闲，4 夹爪正在闭合，5
             ///< 夹爪正在张开，6 夹爪闭合过程中遇到力控停止
