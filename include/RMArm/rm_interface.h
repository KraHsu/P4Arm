...
#ifndef RM_FUNCTION_H
#define RM_FUNCTION_H
 ...
 * @param ip 机械臂的ip地址
 * @param port 机械臂的端口号
 * @return rm_robot_handle*
 * 创建成功后，返回机械臂控制句柄，连接成功句柄id大于0，连接失败句柄id返回-1，达到最大连接数5创建失败返回空
 */
...
/**
 * @brief 机械臂事件回调函数注册
 * @details 当机械臂返回运动到位指令或者文件运行结束指令时会有数据返回
 * @attention 单线程无法使用该函数获取到位信息
 * @param handle 机械臂控制句柄
 * @param event_callback 机械臂事件回调函数，该回调函数接收rm_event_push_data_t类型的数据作为参数，没有返回值。
 */
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_generate_auto_tool_frame(rm_robot_handle* handle, const char* name, float payload, float x,
                                                    float y, float z);
/**
 * @brief 手动设置工具坐标系
 *
 ...
 *
 * @param handle 机械臂控制句柄
 * @param frame_names 返回的工具坐标系名称字符数组
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_tool_envelope(rm_robot_handle* handle, const char* tool_name,
 ...
 */
RM_INTERFACE_EXPORT int rm_movej(rm_robot_handle* handle, const float* joint, int v, int r, int trajectory_connect,
                                 int block);
/**
 * @brief 笛卡尔空间直线运动
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
 */
RM_INTERFACE_EXPORT int rm_movel(rm_robot_handle* handle, rm_pose_t pose, int v, int r, int trajectory_connect,
                                 int block);
/**
 * @brief 样条曲线运动
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
 */
RM_INTERFACE_EXPORT int rm_moves(rm_robot_handle* handle, rm_pose_t pose, int v, int r, int trajectory_connect,
                                 int block);
/**
 * @brief 笛卡尔空间圆弧运动
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
 */
RM_INTERFACE_EXPORT int rm_movec(rm_robot_handle* handle, rm_pose_t pose_via, rm_pose_t pose_to, int v, int r, int loop,
 ...
 * @return int 函数执行的状态码。
            ...
            - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
 */
RM_INTERFACE_EXPORT int rm_movej_p(rm_robot_handle* handle, rm_pose_t pose, int v, int r, int trajectory_connect,
                                   int block);
/**
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_arm_current_trajectory(rm_robot_handle* handle, rm_arm_current_trajectory_e* type,
                                                      float* data);
/** @} */  // 结束组的定义
 ...
 */
RM_INTERFACE_EXPORT int rm_set_pos_step(rm_robot_handle* handle, rm_pos_teach_type_e type, float step, int v,
                                        int block);
/**
 * @brief 当前工作坐标系下，姿态步进
 *
 ...
 * @attention 参考坐标系默认为当前工作坐标系，可调用rm_set_teach_frame修改为工具坐标系，
 */
RM_INTERFACE_EXPORT int rm_set_ort_step(rm_robot_handle* handle, rm_ort_teach_type_e type, float step, int v,
                                        int block);
/**
 * @brief 切换示教运动坐标系
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_controller_state(rm_robot_handle* handle, float* voltage, float* current,
                                                float* temperature, int* err_flag);
/**
 * @brief 设置机械臂电源
 *
 * @param handle 机械臂控制句柄
 ...
 *
 * 机械臂控制器提供IO端口，用于与外部设备交互。数量和分类如下所示：
 *
 * @image html io.png
 * @{
 */
 ...
 *
 * 机械臂末端工具端提供多种IO端口，用于与外部设备交互。数量和分类如下所示：
 * @image html tool_io.png
 * @{
 */
/**
 * @brief 设置工具端数字 IO 输出
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_tool_IO_state(rm_robot_handle* handle, int* mode, int* state);
/**
 * @brief 设置工具端电源输出
 *
 * @param handle 机械臂控制句柄
 ...
 * @brief 自动设置六维力重心参数
 * @details
 设置六维力重心参数，六维力重新安装后，必须重新计算六维力所受到的初始力和重心。分别在不同姿态下，获取六维力的数据，
 ...
 * @brief 手动标定六维力数据
 * @details
 六维力重新安装后，必须重新计算六维力所受到的初始力和重心。该手动标定流程，适用于空间狭窄工作区域，以防自动标定过程中
...
/**
 * @defgroup OneForceSensor 末端传感器一维力
 *
 ...
 * @param handle 机械臂控制句柄
 * @param mode 拖动示教模式
 0-电流环模式，1-使用末端六维力，只动位置，2-使用末端六维力，只动姿态，3-使用末端六维力，位置和姿态同时动
 * @param singular_wall
 仅在六维力模式拖动示教中生效，用于指定是否开启拖动奇异墙，0表示关闭拖动奇异墙，1表示开启拖动奇异墙
 * @return int 函数执行的状态码。
            - 0: 成功。
            - 1: 控制器返回false，传递参数错误或机械臂状态发生错误。
            - -1: 数据发送失败，通信过程中出现问题。
 ...
 * @note 失败的可能原因:
            ...
            - 使用六维力模式拖动示教时，当前已处于奇异区
 */
RM_INTERFACE_EXPORT int rm_start_multi_drag_teach_new(rm_robot_handle* handle, rm_multi_drag_teach_t teach_state);
/**
 * @brief 设置电流环拖动示教灵敏度
 *
 * @param handle 机械臂控制句柄
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_set_force_position_new(rm_robot_handle* handle, rm_force_position_t param);
 ...
 * @param handle 机械臂控制句柄
 * @param hand_angle 手指角度数组，6个元素分别代表6个自由度的角度，范围：0~1000.
 ...
 *
 * 睿尔曼机械臂在控制器和末端接口板上各提供一个RS485通讯接口，这些接口可通过接口配置为标准的Modbus RTU模式。
 ...
 * @param timeout
 超时时间，单位百毫秒。。对Modbus设备所有的读写指令，在规定的超时时间内未返回响应数据，则返回超时报错提醒。超时时间不能为0，若设置为0，则机械臂按1进行配置。
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_input_status(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                             int* data);
/**
 * @brief 读保持寄存器
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_holding_registers(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                                  int* data);
/**
 * @brief 读输入寄存器
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_input_registers(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                                int* data);
/**
 * @brief 写单圈数据
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_write_single_coil(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                             int data);
/**
 * @brief 写单个寄存器
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_write_single_register(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                                 int data);
/**
 * @brief 写多个寄存器
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_write_registers(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                           int* data);
/**
 * @brief 写多圈数据
 *
 ...
 *
 * @param handle 机械臂控制句柄
 * @param params 多圈数据读取参数结构体，要读的线圈的数量 8< num <= 120，该指令最多一次性支持读 120 个线圈数据， 即 15
 个 byte
 * @param data 返回线圈状态，数据类型：int8
 * @return int 函数执行的状态码。
            - 0: 成功。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_multiple_coils(rm_robot_handle* handle, rm_peripheral_read_write_params_t params,
                                               int* data);
/**
 * @brief 读多个保存寄存器
 *
 * @param handle 机械臂控制句柄
 * @param params 多个保存寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12 个寄存器数据，
 即 24 个 byte
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_multiple_holding_registers(rm_robot_handle* handle,
                                                           rm_peripheral_read_write_params_t params, int* data);
/**
 * @brief 读多个输入寄存器
 *
 * @param handle 机械臂控制句柄
 * @param params 多个输入寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12 个寄存器数据，
 即 24 个 byte
 * @param data 返回寄存器数据，数据类型：int8
 * @return int 函数执行的状态码。
            - 0: 成功。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_read_multiple_input_registers(rm_robot_handle* handle,
                                                         rm_peripheral_read_write_params_t params, int* data);
/** @} */  // 结束组的定义
 ...
 * 高速网口的透传周期最快也可到 10ms，不过在使用该高速网口前，需要使用指令打开配置。另外 I 系列有线网口周期最快可达
 * 2ms。
 *
 * @{
 */
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_force_position_move_joint(rm_robot_handle* handle, const float* joint, int sensor, int mode,
                                                     int dir, float force, bool follow);
/**
 * @brief 透传力位混合补偿-位姿方式
 *
 * @param handle 机械臂控制句柄
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_force_position_move_pose(rm_robot_handle* handle, rm_pose_t pose, int sensor, int mode,
                                                    int dir, float force, bool follow);
/**
 * @brief 透传力位混合补偿-新参数
 *
 ...
 * @param errline 若运行失败，该参数返回有问题的工程行数，err_line 为 0，则代表校验数据长度不对，err_line 为
 -1，则代表无错误
 * @return int 函数执行的状态码。
            - 0: 成功。
            - 1: 控制器返回false，传递参数错误或机械臂状态发生错误。
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_program_trajectory_list(rm_robot_handle* handle, int page_num, int page_size,
                                                       const char* vague_search, rm_program_trajectorys_t* trajectorys);
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_global_waypoints_list(rm_robot_handle* handle, int page_num, int page_size,
                                                     const char* vague_search, rm_waypoint_list_t* point_list);
/** @} */  // 结束在线编程组的定义

/**
 * @defgroup UdpConfig UDP主动上报
 *
 * 睿尔曼机械臂提供 UDP 机械臂状态主动上报接口，使用时，需要和机械臂处于同一局域网络下，通过设置主动上报配置接口的目标
 * IP或和机械臂建立 TCP 连接， 机械臂即会主动周期性上报机械臂状态数据，数据周期可配置，默认
 * 5ms。配置正确并开启三线程模式后，通过注册回调函数可接收并处理主动上报数据。
 * @{
 */
/**
 ...
 * 机械臂的拖动操作不会超出预设的范围。
 * @attention
 * 虚拟墙功能目前支持长方体和球体两种形状，并仅在上述两种示教模式下有效。在其他操作模式下，此功能将自动失效。因此，请确保在正确的操作模式
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_given_electronic_fence_config(rm_robot_handle* handle, const char* name,
                                                             rm_fence_config_t* config);
/**
 * @brief 查询所有几何模型参数
 *
 * @param handle 机械臂控制句柄
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_get_electronic_fence_list_infos(rm_robot_handle* handle, rm_fence_config_list_t* config_list,
                                                           int* len);
/**
 * @brief 设置电子围栏使能状态
 *
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 * @note
 电子围栏功能通过精确设置参数，确保机械臂的轨迹规划、示教等运动均在设定的电子围栏范围内进行。当机械臂的运动轨迹可能超出电子围栏的界限时，
 ...
 */
RM_INTERFACE_EXPORT int rm_algo_inverse_kinematics(rm_robot_handle* handle, rm_inverse_kinematics_params_t params,
                                                   float* q_out);
/**
 * @brief 正解算法接口
 *
 ...
 */
RM_INTERFACE_EXPORT rm_pose_t rm_algo_rotate_move(rm_robot_handle* handle, const float* const curr_joint,
                                                  int rotate_axis, float rotate_angle, rm_pose_t choose_axis);
/**
 * @brief 计算沿工具坐标系运动位姿
 *
 * @param handle 机械臂控制句柄，连接机械臂时传入机械臂控制句柄，不连接时传入NULL
 ...
 */
RM_INTERFACE_EXPORT rm_pose_t rm_algo_cartesian_tool(rm_robot_handle* handle, const float* const curr_joint,
                                                     float move_lengthx, float move_lengthy, float move_lengthz);
/**
 ...
 */
RM_INTERFACE_EXPORT rm_pose_t rm_algo_pose_move(rm_robot_handle* handle, rm_pose_t poseCurrent,
                                                const float* deltaPosAndRot, int frameMode);
/**
 * @brief 末端位姿转成工具位姿
 *
 * @param handle 机械臂控制句柄
