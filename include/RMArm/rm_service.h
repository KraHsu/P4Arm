...
class RM_Service {
 public:
   ...
   * @return int 函数执行的状态码。
              ...
              - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
   */
  RM_INTERFACE_EXPORT int rm_movec(rm_robot_handle* handle, rm_pose_t pose_via, rm_pose_t pose_to, int v, int r,
                                   int loop, int trajectory_connect, int block);
  /**
   ...
   * @return int 函数执行的状态码。
              ...
              - -1: 数据发送失败，通信过程中出现问题。
   */
  RM_INTERFACE_EXPORT int rm_movep_canfd(rm_robot_handle* handle, rm_pose_t pose, bool follow, int trajectory_mode = 0,
                                         int radio = 0);
  /**
   * @brief 关节空间跟随运动
   * @param handle 机械臂控制句柄
   ...
   * @return int 函数执行的状态码。
              ...
              - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
   */
  RM_INTERFACE_EXPORT int rm_set_IO_mode(rm_robot_handle* handle, int io_num, int io_mode, int io_speed_mode = 0,
                                         int io_speed = 0);
  /**
   * @brief 设置数字IO输出
   *
  ...
  RM_INTERFACE_EXPORT int rm_get_IO_state(rm_robot_handle* handle, int io_num, int* state, int* mode,
                                          int* io_speed_mode = nullptr, int* io_speed = nullptr);
  /**
   * @brief 获取所有 IO 输入状态
   *
   * @param handle 机械臂控制句柄
   ...
   * @return int 函数执行的状态码。
              ...
              - -4:超时
   */
  RM_INTERFACE_EXPORT int rm_set_gripper_pick_on(rm_robot_handle* handle, int speed, int force, bool block,
                                                 int timeout);
  /**
   * @brief 设置手爪达到指定位置
   ...
   * @brief 力位混合控制
   * @details
   在笛卡尔空间轨迹规划时，使用该功能可保证机械臂末端接触力恒定，使用时力的方向与机械臂运动方向不能在同一方向。
   ...
   * @param handle 机械臂控制句柄
   * @param hand_angle
   手指角度数组，最大表示范围为-32768到+32767，按照灵巧手厂商定义的角度做控制，例如因时的范围为0-2000
   ...
   *
   * @param handle 机械臂控制句柄
   * @param params 多个保存寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12
   个寄存器数据， 即 24 个 byte
   ...
   *
   * @param handle 机械臂控制句柄
   * @param params 多个输入寄存器读取参数结构体，要读的寄存器的数量 2 < num < 13，该指令最多一次性支持读 12
   个寄存器数据， 即 24 个 byte
   * @param data 返回寄存器数据，数据类型：int8
   * @return int 函数执行的状态码。
              - 0: 成功。
   ...
   * @return int 函数执行的状态码。
              ...
              - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
   */
  RM_INTERFACE_EXPORT int rm_force_position_move_joint(rm_robot_handle* handle, const float* joint, int sensor,
                                                       int mode, int dir, float force, bool follow);
  /**
   * @brief 透传力位混合补偿-位姿方式
   *
   * @param handle 机械臂控制句柄
  ...
  RM_INTERFACE_EXPORT int rm_get_program_trajectory_list(rm_robot_handle* handle, int page_num, int page_size,
                                                         ...
                                                         rm_program_trajectorys_t* trajectorys);
   ...
   * @return int 函数执行的状态码。
              ...
              - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
   */
  RM_INTERFACE_EXPORT int rm_get_electronic_fence_list_names(rm_robot_handle* handle, rm_fence_names_t* names,
                                                             int* len);
  /**
   * @brief 查询指定几何模型参数
   *
   ...
   * @return int 函数执行的状态码。
              ...
              - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
   */
  RM_INTERFACE_EXPORT int rm_get_electronic_fence_list_infos(rm_robot_handle* handle,
                                                             rm_fence_config_list_t* config_list, int* len);
  /**
   * @brief 设置电子围栏使能状态
   *
   * @param handle 机械臂控制句柄
